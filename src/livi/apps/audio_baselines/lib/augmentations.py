        ...
        for i in ids:
            if augs[i] == "polarity" and self.conf.polarity.p > 0:
                ...
                for _ in range(nf):
                    ...
                    xeq = torchaudio.functional.equalizer_biquad(xeq, self.sr, freq, gain, Q=q)
                ...
                qtl = 1 - torch.rand(x.size(0), 1, device=x.device) * self.conf.clipping.max_qtl
                thres = tops.tensor_quantile(x.abs(), qtl, dim=-1, keepdim=True)
                mask = torch.rand(x.size(0), 1, device=x.device) < self.conf.clipping.p_soft
    ...
    def cqgram(
        ...
        for i in ids:
            if augs[i] == "specaugment" and self.conf.specaugment.p > 0:
                ...
                for _ in range(n):
                    fpc = torch.rand(y.size(0), 1, 1, device=y.device) * self.conf.specaugment.f_pc
                    flen = (fpc * y.size(1)).clamp(min=1).long()
                    fmax = y.size(1) - flen
                    f0 = (torch.rand_like(fpc) * fmax).long()
                    tpc = torch.rand(y.size(0), 1, 1, device=y.device) * self.conf.specaugment.t_pc
                    ...
                    if self.conf.specaugment.full:
                        cond = ((fids >= f0) & (fids < f0 + flen)) | ((tids >= t0) & (tids < t0 + tlen))
                    else:
                        cond = ((fids >= f0) & (fids < f0 + flen)) & ((tids >= t0) & (tids < t0 + tlen))
                    ydrop = torch.where(cond, 0, ydrop)
                mask = torch.rand(y.size(0), 1, 1, device=y.device) < self.conf.specaugment.p
                ...
                mask = torch.rand(y.size(0), 1, 1, device=y.device) < self.conf.timestretch.p
                ...
                mask = torch.rand(y.size(0), 1, 1, device=y.device) < self.conf.pitchstretch.p
                y = torch.where(mask, ys, y)
                del ys
                ...
                mask = torch.rand(y.size(0), 1, 1, device=y.device) < self.conf.pitchtranspose.p
                ...
                r = julius.lowpass.lowpass_filter(r, self.conf.reqcqt.lpf, zeros=4).unsqueeze(-1)
