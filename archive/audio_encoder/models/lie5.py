...
class WhisperEncoder(nn.Module):
    def __init__(self, device: torch.device | None = None, debug: bool = True, precision: str = "fp32"):
        super().__init__()

        self.debug = debug
        self.device = device if device else ("cuda" if torch.cuda.is_available() else "cpu")

        self.model = WhisperModel.from_pretrained("openai/whisper-large-v3-turbo").encoder
        self.model.half()
        for param in self.model.parameters():
            param.requires_grad = False
        ...
        # 🔧 Compile only the encoder (to avoid overhead on the rest)
        try:
            self.model = torch.compile(self.model, mode="default")  # or "default"
    ...
    def _whisper_forward(self, x: Tensor) -> Tensor:
        ...
        # logger.info(f"[WhisperEncoder] Outputs shape: {outputs.shape}")
...
class LightLIE(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.audio_encoder = WhisperEncoder(device=device, debug=debug, precision=precision)
        ...
        self.logit_scale = nn.Parameter(torch.tensor(math.log(1 / 0.07), device=device, dtype=amp_dtype))
    ...
    def forward(self, audio: Tensor, text: Tensor) -> Tensor:
        # ---- Audio branch ----
        # 1. Get audio hidden states
        # start_time = time.perf_counter()
        whisper_hidden_state = self.audio_encoder(audio)
        # end_time = time.perf_counter()
        # try:
        #    logger.info(f"Whisper: {float(end_time - start_time):.4f} seconds")
        # except Exception as e:
        #    logger.warning(f"Logging Whisper timing failed: {e}")

        # tart_time = time.perf_counter()
        audio_embedding = self.audio_proj(whisper_hidden_state.to(self.device))
        # end_time = time.perf_counter()

        # logger.info(f"Projection: {float(end_time - start_time)}")

        # ---- Text branch ----
        # start_time = time.perf_counter()
        text_embedding = self.text_encoder(text)
        # end_time = time.perf_counter()
        # logger.info(f"Text: {float(end_time - start_time)}")

        if self.debug:
            logger.info(f"[LIE] Input {audio.shape=} {text.shape=} ")
            logger.info(f"[LIE] Audio hidden states {whisper_hidden_state.shape=}")
