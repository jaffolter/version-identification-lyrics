...
class Projection(nn.Module):
    """
    Projection layer that applies a series of linear layers with layer normalization and GELU activation.
    ...
    def __init__(self, d_in: int = 1024, d_out: int = 768, hidden: List[int] | None = [768]):
        ...
        # layers.append(nn.LayerNorm(d_out, elementwise_affine=False))
        self.net = nn.Sequential(*layers)

        # self.net.apply(self.init_projection)

    def init_projection(self, m, use_relu=False):
        if isinstance(m, nn.Linear):
            if use_relu:
                nn.init.kaiming_uniform_(m.weight, a=math.sqrt(5))  # ReLU
            else:
                nn.init.kaiming_uniform_(m.weight, a=math.sqrt(2))  # GELU or none
            if m.bias is not None:
                nn.init.constant_(m.bias, 0)
            ...
            try:
                ...
                logger.warning(f"Failed to initialize LayerNorm: {e}")
...
class WhisperEncoderTrainable(nn.Module):
    ...
    def __init__(self, debug: bool = False, pooling: str = "mean", last_k: int = 4, device: Optional[str] = "cuda:0"):
        """
        Args:
            debug (bool, optional): Whether to enable debug mode (hence logging). Defaults to False.
            pooling (str, optional): Pooling method to use. Defaults to "mean". Options are "mean" or "attention".
        ...
        for blk in self.model.layers[-last_k:]:
